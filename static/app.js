...
const COPY = {
  "zh-CN": {
    ...
    connectEngine: "连接 ComfyUI 推理引擎", offline: "服务离线", autoSchedule: "自动调度", nodePending: "节点待分配", online: "在线", nodeOffline: "离线", disabled: "已停用", busy: "执行中",
    ...
    reuse: "回填到发送区", edit: "修改", cancel: "取消", delete: "删除", deleteRecord: "删除记录", downloadVideo: "下载 MP4", downloadAudio: "下载 FLAC", videoReady: "视频已生成", musicReady: "音乐已生成",
    ...
    deleteConfirm: "删除后将同时清理任务记录、上传素材和生成产物，确认继续？", fillLoading: "正在读取原始文件", fillDone: "已回填到发送区", fileUnavailable: "参考文件已不存在，无法完整回填",
  ...
  en: {
    ...
    connectEngine: "Connecting to ComfyUI", offline: "Service offline", autoSchedule: "Auto", nodePending: "Awaiting node", online: "Online", nodeOffline: "Offline", disabled: "Disabled", busy: "Running",
    ...
    reuse: "Fill composer", edit: "Edit", cancel: "Cancel", delete: "Delete", deleteRecord: "Delete record", downloadVideo: "Download MP4", downloadAudio: "Download FLAC", videoReady: "Video generated", musicReady: "Music generated",
    ...
    deleteConfirm: "This removes the record, uploaded files, and generated output. Continue?", fillLoading: "Loading source files", fillDone: "Filled into the composer", fileUnavailable: "A source file is unavailable and cannot be restored",
...
function applyStaticLocale() {
  ...
  setText("#writeLyrics span", "AI lyrics");
  ...
  setText("#nodeDialogTitle", "ComfyUI nodes");
  setText(".node-dialog-header p", "Node configuration and health checks");
  ...
  setText('label[for="nodeUrl"]', "ComfyUI API URL");
  ...
  setTitle("#writeLyrics", "AI lyrics");
  ...
  setTitle("#openNodeManager", "Manage ComfyUI nodes");
  ...
  setControlTitle("comfyNode", "ComfyUI node");
...
function renderNodeOptions(nodes = state.nodes) {
  ...
  state.nodes.forEach((node) => {
    const stateLabel = !node.healthy ? t("nodeOffline") : node.busy ? t("busy") : t("online");
    options.push(`<option value="${escapeHtml(node.id)}"${node.healthy ? "" : " disabled"}>${escapeHtml(node.name)} · ${stateLabel}</option>`);
  });
...
function renderHealthState(nodes = state.nodes, queueDepth = state.queue.length) {
  ...
  const message = state.locale === "en"
    ? `${online}/${state.nodes.length} nodes online · capacity ${online} · queue ${queueDepth}`
    : `${online}/${state.nodes.length} 节点在线 · 并发 ${online} · 队列 ${queueDepth}`;
...
function renderManagedNodes() {
  ...
  el("nodeList").innerHTML = state.managedNodes.length ? state.managedNodes.map((node) => {
    ...
    const activity = node.busy
      ? ` · 任务 ${escapeHtml((node.running_job_id || "受限任务").slice(0, 8))}`
    ...
    return `<div class="node-row" data-node-id="${escapeHtml(node.id)}">
      ...
      <div class="node-row-copy"><div><strong>${escapeHtml(node.name)}</strong><span>${status.label}${activity}</span></div><code title="${escapeHtml(node.url)}">${escapeHtml(node.url)}</code><small>ID ${escapeHtml(node.id)}${node.last_checked ? ` · ${formatTime(node.last_checked, true)} 检测` : ""}</small></div>
...
function updateModelUi() {
  ...
  el("optimizePrompt").title = state.locale === "en" ? music3 ? "AI arrangement" : "Optimize prompt" : music3 ? "AI 编曲" : "优化提示词";
  el("optimizePrompt").setAttribute("aria-label", el("optimizePrompt").title);
  el("optimizePromptLabel").textContent = state.locale === "en" ? music3 ? "AI arrangement" : "Optimize prompt" : music3 ? "AI 编曲" : "优化提示词";
...
el("writeLyrics").addEventListener("click", () => assistMusic("lyrics"));
