...
class NodeRegistry:
    ...
    def _initialize(self) -> None:
        with self._lock, self._connect() as connection:
            ...
            if re.search(r"url\s+TEXT\s+NOT\s+NULL\s+UNIQUE", table_sql, re.IGNORECASE):
                connection.executescript(
                    """
                    ...
                    INSERT INTO comfy_nodes_migrated (
                        id, name, url, provider, api_key, workflow_id,
            ...
            if count == 0:
                ...
                connection.execute(
                    ...
                    "(id, name, url, provider, api_key, workflow_id, max_concurrency, "
    ...
    def _validate(
        ...
        if provider == "runninghub":
            ...
            if not workflow_id or len(workflow_id) > 120:
                raise ValueError("RunningHub API 节点必须填写有效的工作流 ID")
    ...
    def create(
        ...
        node = self._validate(
            node_id, name, url, provider, api_key, workflow_id, max_concurrency
        )
        now = utc_now()
        try:
            with self._lock, self._connect() as connection:
    ...
    def update(
        ...
        with self._lock, self._connect() as connection:
            connection.execute(
                ...
                "workflow_id = ?, max_concurrency = ?, enabled = ?, updated_at = ? "
                "WHERE id = ?",
                (
                    node.name,
                    node.url,
