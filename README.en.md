...
### 2026-09-23: Video super-resolution and automatic processing

- Added real, anime, and 3D image and video super-resolution at 2x and 4x.
- Added automatic post-generation super-resolution for ordinary video jobs. The same job continues into super-resolution and returns the final upscaled video.
- Added GPU-aware long-video slicing based on free VRAM, input resolution, and scale, with slice progress, checkpoint recovery, final merge, and intermediate cleanup.
- Increased the ComfyUI and API single-file upload limit to 2048 MB.
- Audited the newly present LTX 2x models and confirmed that they require a latent super-resolution workflow. The current pixel super-resolution flow continues to use the verified image upscalers.

This log covers code, workflows, documentation, tests, and the cloud publishing process. The current workspace state is recorded through 2026-09-23.
